...
class OnboardingPage {
    ...
    detectRetakeMode() {
        ...
        // RETAKE: Дополнительное логирование для отладки
        if (this.isRetakeMode) {
            console.log('🔄 RETAKE: Режим повторного прохождения активирован');
            console.log('🔄 RETAKE: URL параметры:', window.location.search);
            console.log('🔄 RETAKE: Состояние приложения:', {
                forceRetake: this.state.get('onboarding.forceRetake'),
                isOnboardingCompleted: this.state.get('user.profile.isOnboardingCompleted')
            });
        }
    ...
    renderCompletion() {
        ...
        // === RETAKE FIX START ===
        // ИСПРАВЛЕНО: В режиме повторного прохождения ВСЕГДА показываем форму контактов
        // согласно требованиям: "always show contact (email/source) step again"
        if (this.isRetakeMode) {
            return `
                <div class="completion-screen retake-mode">
                    <div class="completion-title">${userName}, обновляем ваш профиль!</div>
                    <div class="completion-description">
                        Спасибо за актуализацию ваших данных. Пожалуйста, подтвердите или обновите ваши контактные данные.
                        <br><br>
                        Это поможет мне давать еще более точные персональные рекомендации.
                    </div>
                    
                    <div class="anna-signature">
                        "Ваше стремление к развитию впечатляет! Обновленный профиль поможет нам работать еще эффективнее."
                        <br><br>
                        — Анна Бусел
                    </div>
    ...
    isContactDataValid() {
        ...
        // === RETAKE FIX START ===
        // ИСПРАВЛЕНО: В режиме повторного прохождения также требуем выбор источника
        // согласно требованиям: "require selecting/confirming source"
        
        // Источник обязателен во всех режимах
        if (!this.contactData.source || this.contactData.source.length === 0) {
            // Автоматический fallback только если мы на этапе завершения и НЕ в режиме retake
            if (this.currentStep > this.totalSteps && !this.isRetakeMode) {
    ...
    async completeOnboarding() {
        ...
        try {
            ...
            if (this.isRetakeMode) {
                ...
                console.log('🔄 RETAKE: Добавлен forceRetake флаг для повторного прохождения');
                console.log('🔄 RETAKE: Данные для отправки:', {
                    email: onboardingData.email,
                    source: onboardingData.source,
                    forceRetake: onboardingData.forceRetake
                });
            }
