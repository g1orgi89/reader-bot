...
class ReportsPage {
    ...
    onShow() {
        ...
        // Если отчет уже загружен, НЕ показываем лоадер, сразу рендерим отчет
        if (this.reportsLoaded && this.weeklyReport) {
            this.rerender();
            return;
        }

        // Если идет загрузка, просто ждём
        if (this.reportsLoading) {
            this.rerender();
            return;
        }

        // Если нужно загрузить отчет — сбрасываем флаги и запускаем загрузку
        this.reportsLoaded = false;
        this.reportsLoading = true;
        this.rerender();
