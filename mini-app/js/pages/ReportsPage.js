...
class ReportsPage {
    constructor(app) {
        ...
        // ✅ НОВОЕ: State для месячных отчётов
        this.monthlyReports = [];
        this.monthlyReportsLoading = false;
        this.monthlyReportsLoaded = false;
        this.selectedMonthlyReport = null; // Для детального просмотра
    ...
    renderAIAnalysis() {
        ...
        // ✅ Format AI text into paragraphs (every 3 sentences)
        const formattedAnalysisText = this.formatAIText(analysisText, 3);
        ...
        // Use our own escapeHtml for tone text
        const toneChip = emotionalTone ? `
            <div class="ai-tone-chip">
                ${toneEmoji ? `${toneEmoji} ` : ''}${this.escapeHtml(emotionalTone)}
            </div>
        ` : '';
        ...
        return `
            <div class="ai-insight">
                ...
                <div class="ai-text">${formattedAnalysisText}</div>
            </div>
        `;
    }
   ...
   async onShow() {
       ...
       // ✅ НОВОЕ: Регистрируем глобальную ссылку для onclick handlers
