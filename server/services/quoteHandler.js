...
Верни ТОЛЬКО чистый JSON без markdown кода или комментариев:
{
  ...
  async _updateUserStatistics(userId, author) {
   ...
   /**
   * Генерировать разнообразный ответ в стиле Анны Бусел с рекомендациями
   * @param {ParsedQuote} parsedQuote - Распарсенная цитата
   * @param {QuoteAnalysis} analysis - Анализ цитаты
  ...
  async generateAnnaResponse(parsedQuote, analysis, todayCount, userId) {
    ...
    // Собираем синонимы и варианты фраз
    const greetings = [
      "✨ Прекрасная цитата!",
      "🌟 Вдохновляющий выбор!",
      "💭 Глубокая мысль!",
      "📖 Как интересно!",
      "🔖 Отличное дополнение к дневнику!"
    ];

    const diaryLines = [
      "Сохранила в ваш дневник.",
      "Добавила в вашу коллекцию.",
      "Записала в дневник мудростей.",
      "Ваш дневник стал богаче!",
    ...
    const authorLines = isAuthorMe
      ...
      ? [
          ...
          `Выбирая ${author}, вы вдохновляете и меня!`,
          `Всегда интересные мысли у ${author}.`
        ]
      : [
          "Ваши собственные мысли всегда ценны.",
          "Люблю, когда вы делитесь своими идеями!",
    ...
    // Инсайт — всегда индивидуальный, если есть
    const insightLine = analysis && analysis.insights ? analysis.insights : "";

    // Редко — поздравление с юбилеем
    const milestone =
      weekQuotes > 0 && weekQuotes % 10 === 0
        ? `🎉 Поздравляю с ${weekQuotes}-й цитатой на этой неделе!`
        : "";
    ...
    if (Math.random() < 0.3) {
      const recommendation = await this._getBookRecommendation(
        analysis.category,
        isClassicAuthor
      );
      if (recommendation) {
        summaryParts.push(`💡 ${recommendation}`);
      }
    }

    // Поощрение за активность (как и раньше)
    if (todayCount >= 5) {
      summaryParts.push(
    ...
    // Удаляем пустые элементы и собираем финальный ответ
